	...
	this.remove = function(resource) {
		if (resource !== null) {
            ...
            for(var i=0; i<resources.length;i++) {
                //Check if we still have the resource in pool 
                //this could be due to a resource dying when still 
                //in the available pool and the program using
                //common-pool detecting it and calling remove()
                if(resources[i].resource.pid === resource.pid) {
